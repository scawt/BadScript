...
'PCR check variables
    ...
    Dim PCRCompleteByPatient As Integer
    PCRCompleteByPatient = 0
    Dim PCRIncompleteByPatient As Integer
    PCRIncompleteByPatient = 0
    Dim PCRTotalByPatient As Integer
    PCRTotalByPatient = 0
    Dim PCRPercentByPatient As Single
...
'Misc Variables
    ...
    Do While (ThisWorkbook.Sheets("OCRA").Range("A" & (intCount)).Value) <> ""
        ...
        Else
            ...
            PCRIncompleteByPatient = PCRIncompleteByPatient + 1
        End If
    ...
    Do While Range("A" & intCount).Value <> ""
        'count the completed PCRs bulled from AIMS
        PCRCompleteByPatient = PCRCompleteByPatient + 1
        'read values from current sheet
    ...
    PCRTotalByPatient = PCRCompleteByPatient + PCRIncompleteByPatient - 1      'subtract 1 because the loop the Incomplete number comes from cycles one more time than there are incomplete PCRs
    If PCRTotalByPatient < 0 Then
        PCRTotalByPatient = 0                                                  'hide my shameful -1
    End If
    ThisWorkbook.Sheets("Stats").Range("B4").Value = PCRTotalByPatient
    ThisWorkbook.Sheets("Stats").Range("B5").Value = PCRCompleteByPatient
    If PCRTotalByPatient = 0 Then
        ThisWorkbook.Sheets("Stats").Range("B6").Value = "No Data"
    Else
        ThisWorkbook.Sheets("Stats").Range("B6").Value = Int((PCRCompleteByPatient / PCRTotalByPatient) * 100) & "%"
    End If
